...
int WASAPICapture::init(WASAPICapture::CAPTURE_TYPE t)
{
	...
	else if (t == MICROPHONE) 
	{
		hr = m_enumerator->GetDefaultAudioEndpoint(eCapture, eMultimedia, m_device.GetAddressOf());
		if (FAILED(hr))
		{
			printf("[WASAPICapture] Failed to create device.\n");
			return -1;
	...
	hr = m_audioClient->Initialize(AUDCLNT_SHAREMODE_SHARED, t == CAPTURE_TYPE ::MICROPHONE ?0:AUDCLNT_STREAMFLAGS_LOOPBACK, m_hnsActualDuration, 0, m_mixFormat, NULL);
	if (FAILED(hr)) 
	{
...
void WASAPICapture::setCallback(const PacketCallback &callback)
{
...
int WASAPICapture::capture()
{
	...
	while (packetLength > 0) 
	{
		...
		//auto sleeptime = std::chrono::microseconds((long long)(std::chrono::microseconds::period::den * numFramesAvailable * 1.0 / m_mixFormat->nSamplesPerSec)/2);
		//std::cout << "sleep time:" << sleeptime.count() / 1000000.0<<" seconds" << std::endl;
